...
<% int id =0;
	  ...
	  <p style = 'font-size :25px; padding :0px; margin:0px; margin-top:20px; text-align :center;'>Order History <span>&#8669;</span></p>
		...
		<tr><th> OrderID</th> <th>Symbol</th><th>InvestmentType</th><th>OrderType</th><th>OrderOption</th><th>Quantity</th>	<th>TimeStamp</th>	</tr>
		<% 
			for(OrderModel s: orders){
				%>
				<tr><td> <%=s.getId() %></td> <td><%=s.getStockName() %></td><td><%=s.getInvestmentType() %></td><td><%=s.getOrderType() %></td><td><%=s.getOrderOption() %></td><td><%=s.getQty() %></td><td><%=s.getDate() %></td>	</tr>
				<%
			}
		}
		%>
