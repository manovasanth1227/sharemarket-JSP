...
public class register extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(userEmail.isEmpty()) {
			isValid = false;
			out.println("Enter the valid email!!</br>");
		}
		if(userName.isEmpty()) {
			isValid = false;
			out.println("Enter the valid name!!</br>");
		}
		...
		if(isValid) {
			int res = userRegistrationController.createUser(userEmail,userName, adharNum, panNum ,  bank , password );
			if(res!=-1) {
				out.println("Resgistration Successfull <br/>");
				out.println("Your User ID ==>"+ res+" <br/>");
				out.println("<a href='/ShareMarketJSP/JSP/index.jsp'>Go to Home Screen</a>");
			}
		}else {
			out.println("<a href='/ShareMarketJSP/HTML/register.html'>Go to Register  Screen</a>");
		}
