	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("userid")!=null) {
			id =Integer.parseInt(session.getAttribute("userid") + "");
			 ...
			 response.sendRedirect("/ShareMarketJSP/HTML/loginFail.html");
		}
		...
		if(!request.getParameter("deposit").isEmpty()) {
			...
			if(check==1) {
				response.sendRedirect("/ShareMarketJSP/HTML/depositSuccess.html");
			}else {
				response.sendRedirect("/ShareMarketJSP/HTML/loginError.html");
			}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!request.getParameter("withdraw").isEmpty()) {
			...
			if(amt <= balance) {
				...
				if(check==1) {
					response.sendRedirect("/ShareMarketJSP/HTML/withdrawSuccess.html");
				}else {
					response.sendRedirect("/ShareMarketJSP/HTML/loginError.html");
				}
			}
